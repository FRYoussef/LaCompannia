...
import presentacion.vistas.GUIPedido_GCImp;

public class DAO_GCImp implements DAO_GC {
	String BBDD_productos = "src/integracion/bbdd_productos_catering.txt";
	String BBDD_pedidos = "src/integracion/bbdd_pedidos_catering.txt";
	...
	public void modificaPedido(TransferPedido_GC ped, EstadoPedido est, Number precio, List<TransferProducto> productos, Number cantidad) {
		if(ped != null) {
			if(est != null) {
				ped.set_estado(est);
	...
	public void guardarPedidoEnBBDD(TransferPedido_GC ped) {
		if(ped.get_estado() == EstadoPedido.PAGADO) { // SOLO SE ALMACENAN LOS PEDIDOS PAGADOS
			try(BufferedWriter wr = new BufferedWriter(new FileWriter(_filePed.getAbsoluteFile(), true))){
				...
				wr.write("Productos: " + "\n");
				for(TransferProducto p: ped.getProductos()) {
	...
	@Override
	public TransferProducto buscaProducto(int _codProd) { //SI NO LO ENCUENTRA NULL, SINO TRANSFERPRODUCTO.
		TransferProducto tE= null;
		for(int i = 0; i < prod.length; i++) {
			if(_codProd == Integer.parseInt(prod[i][0])) {
				tE = new TransferProducto();

...
public class GUIPedido_GCImp extends GUIPedido_GC {
	...
	public void initGUI(){
		...
		marco.setMaximumSize(new Dimension(650, 450));
		marco.setMinimumSize(new Dimension(650, 450));
		marco.setPreferredSize(new Dimension(650, 450));
		JPanel panel= new JPanel(new BorderLayout());
		
		JPanel superior= new JPanel();
		superior.setLayout(new BoxLayout(superior, BoxLayout.Y_AXIS));
		_sel = new JLabel("Selecciona los productos que desea a�adir pulsandolo en la consulta.");
		...
		marco.addWindowListener(new WindowAdapter() { //Para cerrar el Frame con el aspa.
			public void windowClosing(WindowEvent e) {
				Controlador_GC.getInstancia().eliminarSA_GC();
				marco.dispose();
			}
		});
	}
	...
	public void elegirProductos() {
		...
		if(sel != null) {
			try {
				Controlador_GC.getInstancia().anadirProducto(Integer.parseInt(sel.substring(codigoProd.length(), codigoProd.length() + DAO_GCImp.tam_Codigo_Producto)));
	...
	public String[] ListToCodigoProductoDelPedido(List<Producto> list) {
		String[] str = new String[list.size()];
		for(int i = 0; i < list.size(); i++) {
	...
	public void eliminarProducto() {
		List<Producto> prod = Controlador_GC.getInstancia().listProductos();
			if(prod.size() > 0) {
				...
				if(sel != null) {
					Producto p = Controlador_GC.getInstancia().buscarProductoEnPedido(Integer.parseInt(sel));
					if(p != null) {
	...
	public void finalizarPedido() { // SUPONGO QUE LA CUENTA ES CORRECTA
		String data = (String) JOptionPane.showInputDialog(marco,  "A�ade numero de cuenta y clave separados por un espacio.",  "PAGAR PEDIDO",
		...
		if(data != null && !data.equals("")) {
			if(Controlador_GC.getInstancia().pagar(data)) {
				JOptionPane.showMessageDialog(marco, "El pedido ha sido pagado correctamente, \nNo olvide el codigo que aparece en el RESUMEN del pedido.",
						"PAGADO", JOptionPane.INFORMATION_MESSAGE, new ImageIcon("icons/correcto.png"));
				textArea.setText(textArea.getText() + "\n\n---------------------------------------------------------------------------------");
